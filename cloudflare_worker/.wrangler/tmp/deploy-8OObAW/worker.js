...
var worker_default = {
  async fetch(request, env) {
    ...
    if (path === "/lowest" && (request.method === "GET" || request.method === "POST")) {
      ...
      const ttl = payload.minPrice == null ? cacheNullTtlSeconds : cacheTtlSeconds;
    ...
    if (path === "/lowest/batch" && request.method === "POST") {
      ...
      for (const prodId of missIds) {
        ...
        const ttl = item.minPrice == null ? cacheNullTtlSeconds : cacheTtlSeconds;
        response.headers.set("Cache-Control", `public, max-age=${ttl}`);
